...
function EditarUsuario() {
    ...
    const handleSubmit = async(event) => {
        ...
        if(password !== passwordConfirm){
            openAlert("Error al editar al usuario", "Las contraseñas no coinciden.", "error", null);
            return;
        }
        openAlert("Editando al usuario", "Espere un momento por favor", "loading");
        ...
        imageFile ? formData.append("foto", imageFile) : '';
        formData.append("tipoUsuario", tipoUsuario);
        formData.append("token", cookie);
        try{
            const response = await axios.post(`${endpoint}/usuario/edit/${id}`, formData, {
                headers: {
            ...
            if(error.response !== undefined && error.response.status === 401){
                openAlert("Error al editar al usuario", "No se ha podido editar al usuario, intenta más tarde.", "error", null);
            }
    ...
    useEffect(() => {
        const getUser = async() => {
            try{
                const response = await axios.get(`${endpoint}/usuario/${id}`);
                if(response.data.usuario){
                    setUsuario(response.data.usuario);
                }else{
                    openAlert("Error al cargar usuario", "Ocurrió un error inesperado al cargar el usuario", "error", "/dashboard")
                }
            }catch(error){
                openAlert("Error de conexión", "Ocurrió un error de conexión",  "error", "/dashboard");
            }
        };
        getUser();
    }, [endpoint, id]);
    ...
    useEffect(() => {
        if(usuario){
            setNoEmp(usuario.noEmp);
            setNombre(usuario.nombre);
            setApP(usuario.apP);
            setApM(usuario.apM);
            setEmail(usuario.email);
            setTipoUsuario(usuario.tipoUsuario);
            setImage(`${endpointImage}${usuario.foto}`);
        }
    }, [usuario]);
    return (
        <>
            ...
            <form className="dashboardForm" onSubmit={handleSubmit}>
                ...
                <div className="testimonial-foto" style={!image ? {
                    ...
                    <label htmlFor="profileFoto" className="label-photo">
                        <img src={image ? image : `${endpointLocal}img/img_defecto.webp`} alt="Preview de la foto de perfil" 
                    ...
                    <input type="file" accept="image/*" onChange={handleImageUpload} id="profileFoto" className="file" name="foto"/>
                </div>
                ...
                <div className="inputForm">
                    ...
                    <input type={`${showPassword ? "text" : "password"}`} name="password" id="password" placeholder="Ingresa su contraseña" onChange={(event)=>setPassword(event.target.value)} 
                    />
                ...
                <div className="inputForm">
                    ...
                    <input type={`${showPasswordConfirm ? "text" : "password"}`} name="password" id="passwordVerification" placeholder="Reingresa tu contraseña" onChange={(event)=>setPasswordConfirm(event.target.value)}/>
